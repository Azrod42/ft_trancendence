...
export class ChannelService {
    ...
    async inviteUserChannel (userTrig: string, userToInvite: string, chanId: string) {
        ...
        if (await this.isUserIn(channel.bannedusers, userToInv))
            throw new HttpException("User is blocked in this channel", HttpStatus.CONFLICT,);
    ...
    async kickUserChannel (userTrig: string, userToInvite: string, chanId: string) {
        const channel: Channel = await this.channelRepo.findOneBy({id: chanId});
        const userToKick = await this.usersService.findByDisplayname(userToInvite);
        if (channel.owner == userToKick)
            throw new HttpException("Impossible to kick channel owner", HttpStatus.CONFLICT,);
        if (await this.isUserIn(channel.owners, userTrig) == false)
            throw new HttpException("You are not channel administrator", HttpStatus.CONFLICT,);
        if (await this.isUserIn(channel.channelusers, userToKick) == false)
            throw new HttpException("User is not in this channel", HttpStatus.INTERNAL_SERVER_ERROR,);
        channel.channelusers = await this.removeFormList(channel.channelusers, userToKick);
        await this.channelRepo.save(channel);
        return true;
    }
    async banUserChannel (userTrig: string, userToInvite: string, chanId: string) {
        ...
        if (await this.isUserIn(channel.bannedusers, userToBan))
            throw new HttpException("User is already banned", HttpStatus.INTERNAL_SERVER_ERROR,);
        channel.bannedusers = await this.addList(channel.bannedusers, userToBan)
        channel.channelusers = await this.removeFormList(channel.channelusers, userToBan);
        await this.channelRepo.save(channel);
        return true;
    }

    async unbanUserChannel (userTrig: string, userToInvite: string, chanId: string) {
        const channel: Channel = await this.channelRepo.findOneBy({id: chanId});
        const userToBan = await this.usersService.findByDisplayname(userToInvite);
        if (await this.isUserIn(channel.owners, userTrig) == false)
            throw new HttpException("You are not channel administrator", HttpStatus.CONFLICT,);
        if (await this.isUserIn(channel.bannedusers, userToBan) == false)
            throw new HttpException("User not banned form this channel", HttpStatus.INTERNAL_SERVER_ERROR,);
        channel.bannedusers = await this.removeFormList(channel.bannedusers, userToBan);
        await this.channelRepo.save(channel);
        return true;
    }
    ...
    async leaveChannel (userTrig: string, chanId: string) {
        ...
        if (channel.owner == userTrig) {
            ...
            while (chanOwn[++i]) {};
            if (i <= 1)
                throw new HttpException("Impossible to leave as solo administrator", HttpStatus.CONFLICT,);
            else {
                i = -1;
                while (chanOwn[++i]){
        ...
        channel.channelusers = await this.removeFormList(channel.channelusers, userTrig);
        await this.channelRepo.save(channel);
        return true;
    }
    async addAdministrator (userTrig: string, userToInvite: string, chanId: string) {
        const channel: Channel = await this.channelRepo.findOneBy({id: chanId});
        let chanUsr = [];
        chanUsr = JSON.parse(channel.channelusers);
        const userToInv = await this.usersService.findByDisplayname(userToInvite);
        ...
        if (await this.isUserIn(channel.owners, userToInv))
            throw new HttpException("User is already administrator", HttpStatus.CONFLICT,);
        if (await this.isUserIn(channel.bannedusers, userToInv))
            throw new HttpException("User is ban in this channel", HttpStatus.CONFLICT,);
        channel.owners = await this.addList(channel.owners,userToInv )
        await this.channelRepo.save(channel);
        return true;
    }
    async removeAdministrator (userTrig: string, userToInvite: string, chanId: string) {
        const channel: Channel = await this.channelRepo.findOneBy({id: chanId});
        let chanUsr = [];
        chanUsr = JSON.parse(channel.channelusers);
        const userToInv = await this.usersService.findByDisplayname(userToInvite);
